...
ZIPG_main <-function(
  ...
  if(!is.null(pbWald_list)){
    ...
    d_index0 = pbWald_list$d_index0
    d_star_index0 = pbWald_list$d_star_index0
    d0 = length(d_index0)-1
    d_star0 = length(d_star_index0)-1
    H0_index = pbWald_list$H0_index
    useref = ifelse(is.null(bWald_list$useref),
                    T,bWald_list$useref)
    
    if(is.null(d_index0)){
      X0 = matrix(rep(1,n_sample),n_sample,1)
      d0 = 0
    }else{
      X0 = matrix(X[,d_index0],n_sample,d0+1)
    }
    
    if(is.null(d_star_index0)){
      X_star0 = matrix(rep(1,n_sample),n_sample,1)
      d_star0 = 0
    }else{
      X_star0 = matrix(X_star[,d_star_index0],n_sample,d_star0+1)
    }
